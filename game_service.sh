...
# Create App2 page with different content
echo "Hello from Game Server on $(hostname)" | sudo tee /var/www/html/index.html
echo "[INFO] Game Server page created"
...
# Retry apt-get install if first attempt fails
for i in {1..3}; do
...
[Unit]
Description=Envoy Sidecar Proxy for Game Service
...
echo "[INFO] Game Service sidecar setup complete at $(date)
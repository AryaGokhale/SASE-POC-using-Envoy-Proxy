...
# Start Envoy 
echo "[INFO] Starting Envoy"
nohup sudo envoy -c /etc/envoy/envoy.yaml --log-level info >/var/log/envoy.log 2>&1 &

sleep 3
if ss -tulnp | grep -q 8000; then
  echo "[INFO] Envoy is running and listening on port 8000"
else
  echo "[ERROR] Envoy did not start properly, check /var/log/envoy.log"
fi
